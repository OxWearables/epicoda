...
Note: throughout, the two-sided 95% confidence interval is used. Though the theory generalises, only the 95% confidence interval is currently implemented. 

## Results of Compositional Data Analyses
The results of Compositional Data Analyses are presented graphically, using plots of model predictions at given pairwise balances of parts (`plot_transfers`) as well as model predictions at particular compositions (`forest_plot_comp`). Internally, these both use `predict_fit_and_ci`, which can also be used in a standalone fashion to explore model predictions and confidence intervals on them. 
