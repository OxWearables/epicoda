...
To calculate the compositional mean using `epicoda`, we run: 
```{r}
...
Note that many of the other functions will output messages about imputation of zeroes, and can have `rounded_zeroes` and `det_limit` arguments set. For all of them, it works the same as for `comp_mean`. Also similarly, the default is usually fine, so in the remainder of this vignette they are left at the default. 
...
Another frequent use case is considering this plot with the density separated by groups. For example, to look at different sexes (which in fact don't differ that much):  
```{r}
plot_density_ternary(data = simdata, 
                     parts_to_plot = c("combined_parts", "partD", "partE"),
                     n_bins = 10, # This argument specifies we want to use 10 bins (i.e. 10% of data lies between each pair of contours on the plot.) Default is 5. 
...
list_for_plot <- list("Extra 0.5 hr/day partB" = new_comp, "3.5 hr/day less partD" = new_comp2)
print(list_for_plot)
```
...
forest_plot_comp(composition_list = list_for_plot, 
                 ...
                 comp_labels = comp_labels)
```
The `forest_plot_comp` function can also be used to compare models. To do this, leave `model` at its default `NULL` value, and set `models_list` as a named list of models. For example, to compare `lm_outcome` with a model adjusted for `age_group` only and an unadjusted model: 
```{r}
lm_outcome_unadjusted <- comp_model(type = "linear",
         outcome = "linear_outcome",
         data = simdata,
         comp_labels = comp_labels)
lm_outcome_age_group_only <- lm_outcome <- comp_model(type = "linear",
         outcome = "linear_outcome",
         covariates = c("agegroup"),
         data = simdata,
         comp_labels = comp_labels)
forest_plot_comp(composition_list = list_for_plot, 
                 models_list = list("Unadjusted" = lm_outcome_unadjusted, "Age-adjusted" = lm_outcome_age_group_only, "Age- and sex- adjusted" = lm_outcome), 
                 dataset = simdata, 
                 comp_labels = comp_labels)
```
...
Slightly differently, `forest_plot_comp` is based on the `forestplot` package, so takes arguments specifying the plot in the same way as that function. Again, to specify these the `forestplot` package should itself be loaded. There are several arguments which set particular aspects of the forest plot. In addition, any argument that the basic `forestplot` function can take, which isn't already set, can be set as an argument to `forest_plot_comp`.
```{r}
library(forestplot)
forest_plot_comp(composition_list = list_for_plot, 
                 model = lm_outcome, 
