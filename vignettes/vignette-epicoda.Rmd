...
`simdata` is a (simulated) compositional dataset in the package. It has 5 columns which make up a hypothetical time use composition: "vigorous" (activity), "moderate" (activity), "light" (activity), "sedentary" (behaviour), "sleep". It also contains sex, age group and outcome columns. As the data is simulated, all results are hypothetical. 
```{r}
head(simdata)
```
