...
Note that this property isn't necesssary for the data to be treated as compositional data: the only requirement is that it is the relative, not the absolute, information which is relevant to the problem being considered. For example, vote counts in an election for different parties in different constituencies can be treated as compositional data: although the overall number of votes cast might differ between constituencies, it is the share of the vote going to each party, and not the total number of votes going to that party in a given constitutency, that is relevant. 
...
From the above, it is clear we will want to use the compositional columns together frequently. So we can record their labels here, so we don't need to repeatedly type them: 
```{r}
...
To calculate the compositional mean of columns `"partA", "partB", "partC", "partD", "partE"` using `epicoda`, we run: 
```{r}
...
lm_outcome_age_group_only <- lm_outcome <- comp_model(type = "linear",
         ...
         comp_labels = comp_labels)
forest_plot_comp(composition_list = list_for_plot, 
                 models_list = list("Unadjusted" = lm_outcome_unadjusted, 
                                    "Age-adjusted" = lm_outcome_age_group_only,
                                    "Age- and sex- adjusted" = lm_outcome), 
                 dataset = simdata, 
                 comp_labels = comp_labels)
```
...
To be filled out with example Cox and logistic models. 

### Predicting the fit at different compositions: getting the numbers out 
To be filled with a use of `predict_fit_and_ci`. 
