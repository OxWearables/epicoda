...
If we want to use some of the covariates as factors in R, and the functions to deal with them correctly, we need to tell R they're factors (categorical variables):
```{r}
...
```
The 'No interations to converge' relates to imputing zeroes, and we don't need to worry about it.   
...
data_ilr_impute_zeroes <- transform_comp(data = simdata,
                                   ...
                                   det_limit = 0.0083)
```
There are also a few other things we can do with this function, like specifying a given 'first component' (which can be useful when it conmes to reporting). To learn more, run ?transform_comp.  
...
(It is possible to intepret - additional functionality making this easier by making it possible to generate the different sets of pivot coordinates, wrap the repeated models, and summarise their output to follow!!)  
## Presenting the results
As we saw, the model summary is quite hard to interpret.  
One way to get round this is to look at model predictions. There are various options- we can look at predictions as we hold most of the composition fixed and vary the balance between the final two components. This is what the plot_transfers function does: 
```{r}
plot_transfers(from_component = "sedentary",
               to_component = "vigorous",
               model = lm_outcome ,
