...
#### Note (18.03.2021)
Currently, a conflict between `ggtern` and `ggplot2` means using `ggtern` can stop axis labels appearing on `ggplot2` graphs. Pending a more permanent solutions, these functions remain in the package, but `ggtern` is not automatically loaded and should be used with care. They are not run in this vignette.
...
### Presenting results via model predictions 
An alternative which may sometimes be useful when interpreting models is to look at model-predicted outcomes for different input variable values (rather than model-estimated differences). Functions related to plotting (e.g. the `plot_transfers` function) can do this by setting `terms = FALSE`: 
```{r}
plot_transfers(from_part = "sedentary",
               to_part = "moderate",
               model = lm_BMI ,
