...
```
This function has several possible arguments related to how the transformation is carried out. However, for basic usage most of these arguments can be left at their defaults. 
...
Substitution plots show the model-predicted difference in the outcome (relative to at the compositional mean) as the balance between two parts of the composition varies. In the case of time use/physical behaviours, they are known as isotemporal substitution plots and show the model-predicted difference in the outcome at higher time in one behaviour at the expense of time in another behaviour (the model-predicted difference in outcome for a hypothetical reallocation of time from one behaviour to another). For example: 
```{r}
...
```{r}
pla <- predict_fit_and_ci(model = lm_BMI, new_data = new_comp, comp_labels = c("vigorous", "moderate", "light", "sedentary", "sleep"))
ploga <- predict_fit_and_ci(model = log_outcome, new_data = new_comp, comp_labels = c("vigorous", "moderate", "light", "sedentary", "sleep"))

pcoxa <- predict_fit_and_ci(model = cox_outcome, new_data = new_comp, comp_labels = c("vigorous", "moderate", "light", "sedentary", "sleep"))
pltfa <- predict_fit_and_ci(model = lm_BMI, new_data = new_comp, comp_labels = c("vigorous", "moderate", "light", "sedentary", "sleep"), terms = FALSE)
plogtfa <- predict_fit_and_ci(model = log_outcome, new_data = new_comp, comp_labels = c("vigorous", "moderate", "light", "sedentary", "sleep"), terms = FALSE)

pcoxtfa <- predict_fit_and_ci(model = cox_outcome, new_data = new_comp, comp_labels = c("vigorous", "moderate", "light", "sedentary", "sleep"), terms = FALSE)
devtools::load_all()
```
