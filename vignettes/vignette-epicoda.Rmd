...
Compositional data is data where the information is contained in the *relative*, rather than the absolute, values of the parts[@Filzmoser2018].  
Here, we have one common case: the compositional columns all sum to the same total i.e. they can be represented as proportions of a whole without any loss of information. However, compositional columns do not have to sum to the same total (e.g. vote counts in an election). 

```{r}
head(apply(simdataplain[, c("partA", "partB", "partC", "partD", "partE")], 1, sum))
```
...
Note that in R, we need to set any categorical covariates as factor variables:
```{r}
simdataplain$sex <- as.factor(simdataplain$sex)
simdataplain$agegroup <- as.factor(simdataplain$agegroup)
```
## Investigating the data
A summary statistic that we often wish to calculate is the compositional mean. Mathematically speaking, where there are $n$ individuals in the dataset, and individual $k$ has composition $(part_{k, 1}, part_{k, 2}, ..., part_{k, D}$, this is: 
\begin{equation}
  \mathcal{C}\Bigg(\sqrt[n]{\prod_{k=1}^{n}{part_{k, 1}}}, \sqrt[n]{\prod_{k=1}^{n}{part_{k, 2}}}, ..., \sqrt[n]{\prod_{k=1}^{n}{part_{k, D}}}\Bigg)
\end{equation}
...
```{r}
epicoda::comp_mean(data = simdataplain, # this is the dataset
          comp_labels = comp_labels, # this is the labels of the compositional columns, 
          # which we specified above
...
Note that this functionality is implemented in other R packages, such as `compositions` and `robCompositions`. It is reimplemented here to ensure that future changes to the defaults in those packages do not affect this package.
```{r}
