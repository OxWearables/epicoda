...
`simdata` is a (simulated) compositional dataset in the package. It has 5 columns which make up a hypothetical time use composition: "vigorous" (activity), "moderate" (activity), "light" (activity), "sedentary" (behaviour), "sleep". It also contains sex, age group and outcome columns.
```{r}
head(simdata)
```
...
Many other functions in the package have `rounded_zeroes` and `det_limit` arguments, because they use calculations which need zeroes to be imputed or excluded. For all of them, it works the same as for `comp_mean`. As in this case, the default is usually fine, so in the remainder of this vignette they are left at the default. 
...
Substitution plots show the model-estimated difference in the outcome (relative to at the compositional mean) as the balance between two parts of the composition varies. In the case of time use behaviours, they are known as isotemporal substitution plots and show the model-estimated difference in the outcome at higher time in one behaviour at the expesne of time in another behaviour. For example: 
```{r}
...
#### Notes 
* There is more information on how confidence intervals for this plot are calculated in the [supplementary vignette](vignettes/derivation_of_CIs_used.Rmd).
...
### Getting the numbers out: estimates/ predictions for particular compositions
To get the numbers out for estimates/predictions at particular compositions, we can use the `predict_fit_and_ci` function (which is wrapped in the plotting functions).This takes a data frame argument for `new_data`. 

```{r}
predict_fit_and_ci(model = lm_BMI, dataset = simdata, new_data = new_comp, comp_labels = c("vigorous", "moderate", "light", "sedentary", "sleep"))
```
