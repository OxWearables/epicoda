...
#' for each composition. Please note that with the default `terms = TRUE`
...
#'
#' @param composition_list Named list of compositions. Note each composition
#' should be stored as a data frame. For example, use the output of
#' `change_composition`.
#' @param models_list If `model` is `NULL` (or not set), a named list of models
...
#' @param text_settings An optional argument to specify text formatting. It
#' should be an `fpTxtGp` object (_forestplot_ package).
#' @param plot_log If this is `TRUE`, the x-axis will be log-transformed.
...
forest_plot_comp <- function(
  ...
  if (is.null(text_settings)) {
    text_settings <- fpTxtGp(
      ...
      ticks = gpar(cex = 0.75, fontface = 2)
    )
  }
  ...
  vline_loc <- if (terms) {
    if (type == "cox" || type == "logistic") 1 else 0
  } else {
    NA_real_
  }
  ...
  if (!is.null(model)) {
    ...
    data_frame_for_forest_plot <- rbind(
      data.frame(
        coef = c(NA, vline_loc),
        low = c(NA, vline_loc),
        high = c(NA, vline_loc)
    ...
    if (is.null(models_list)) {
      stop('Either `model` or `models_list` must be set.')
    }
    ...
    for (i in seq_len(nrow(df))) {
      ...
      data_frame_for_forest_plot <- rbind(
        data_frame_for_forest_plot,
        data.frame(coef = coef, low = low, high = high)
      )
    }
    ...
    fp <- forestplot(
      ...
      col = fpColors(box = col_vec, lines = col_vec, zero = "black"),
      boxsize = boxsize,
      ...
    )
  }
