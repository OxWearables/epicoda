...
#'
#' @description
#' This pulls out a table of the coefficients of the covariates only in the
#' model. Confidence intervals are calculated using `confint()`. The function
#' may fail for logistic regression if the _MASS_ package is not installed.
#'
#' @param model Model for which extracting coefficients.
#' @param scale_type Can take value `"lp"` (linear predictors) or `"exp"`
#' (exponentiated- relevant for interpretation of logistic and Cox models).
#' @param level The level argument of the confidence intervals. Passed directly
#' to `stats::confint()`.
#' @inheritParams comp_model
#'
#' @return
#' Table of covariates and their associated coefficients (parameter value with
#' lower and upper confidence interval). Note that, unlike the plotting and some
#' other default model summary functions in R, this uses confidence intervals
#' based on `stats::confint()`.
#' 
...
#'
#' @description
#' This pulls out a table of the coefficients of the models. Confidence
#' intervals are calculated using `confint()`. The function may fail for
#' logistic regression if the _MASS_ package is not installed.
#'
#' @inheritParams tab_covariate_coefs
#' @inheritParams comp_model
#'
#' @return
#' Table of all model coefficients (parameter value with lower and upper
#' confidence interval of the 95%), using the first pivot coordinates for all
#' compositional coefficients.
#' 
#' @examples
#' tab_coefs(
#'   scale_type = "lp", # This argument can be "lp" or "exp" and determines whether
...
tab_coefs <-
  function(scale_type = "lp",
    ...
    scale_type <- match.arg(scale_type, c("exp", "lp"))
    ...
    for (part in comp_labels) {
      ...
      colnames(toc) <- "fit"
      tocint <- confint(current_model, level = level, complete = FALSE)
      all <- cbind(toc, tocint)
