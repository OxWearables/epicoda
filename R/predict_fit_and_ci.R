...
#'
#' @details
#' Note that confidence intervals use the t-distribution with the appropriate
#' degrees of freedom for linear regression, and the z-distribution for logistic
#' and Cox regression, to match the behaviour of `summary()` for these model
#' objects. As long as there are a reasonable number of samples (at least 30,
#' say) the difference between the two is negligible.
#'
#' @param model Model to use for predictions.
#' @param new_data Data for predictions.
#' @param terms Are estimates for differences in outcome associated with
#' differences in compositional variables? If `terms = TRUE` all estimates and
#' plots will be for difference in outcome associated with differences in the
#' compositional variables. If `terms = FALSE`, `fixed_values` is used to set
...
#' `terms = TRUE` estimates are odds ratios; for logistic regression models with
#' `terms = FALSE` estimates are probabilities (i.e. predictions on the response
#' scale).
#' @param fixed_values If `terms = FALSE`, this gives the fixed values of the
...
#' makes no difference to the output if `terms = TRUE`.
...
#'
#' @importFrom stats df.residual formula predict qnorm qt vcov
#'
#' @examples
...
predict_fit_and_ci <- function(
  ...
  z_value <- qnorm(0.975) # Currently approximately 1.96 for 95% CI; To be updated to allow user-specified CIs by specification of level
  ...
  transf_labels <- transf_labels(comp_labels, transformation_type = "ilr", part_1 = part_1)
  ...
  cm_transf_df <- get_cm_from_model(
    model = model,
    comp_labels = comp_labels,
    transf_labels = transf_labels
  )$cm_transf_df
  ...
  if (!(is.null(fixed_values))) {
    ...
    fixed_values <- generate_fixed_values(dataset_ready, comp_labels)
    fixed_values <- cbind(fixed_values, cm)
  }
  ...
  if (type == "logistic") {
    ...
    if (terms) {
      ...
      dNew$lower_CI <- dNew$fit * exp(-(z_value*dNew$se.fit))
      dNew$upper_CI <- dNew$fit * exp(+(z_value*dNew$se.fit))
