...
#'
#' @param nrow Number of rows of transformation matrix to be created (= number
#' of parts in composition).
...
#'
#' @param units What should the units of the compositional variables be in any
#' output? Currently available are "unitless" (where working in terms of
#' proportions), `"hr/day"`, `"hr/wk"`, `"min/day"`, `"min/wk"`, `"unspecified"`
#' and `"specified"`, in which case the `specified_units` argument should be
#' set. Note that this doesn't specify the input units, as this is not relevant
#' for any function.
#' @param specified_units If units are being specified via the composition sum,
...
process_units <- function(units, specified_units) {
  ...
  if (units == "specified") {
    ...
    comp_sum <- as.double(specified_units[2])
...
#' @param det_limit Detection limit if zeroes are to be imputed. This is needed
#' when `rounded_zeroes` is `TRUE`. It should be the minimum measurable value in
#' the compositional columns of data, and should be on the same scale as the
#' (input) compositional columns.nIf `rounded_zeroes` is `TRUE` and there are
  ...
  function(data,
    ...
    if (any(comp_data == 0, na.rm = TRUE)) {
      if (rounded_zeroes) {
        ...
        comp_data_nonans <-
          comp_data[complete.cases(comp_data[, comp_labels]), ] # need to avoid missing values in data passed to zCompositions::lrEM
        comp_data_nans <-
          comp_data[!complete.cases(comp_data[, comp_labels]), ]
        ...
        capture.output(comp_data_nonans[, comp_labels] <-
                         lrEM(
...
#' Process axis labels
#'
#' @param label This is the axis label that already exists. To suppress the
#' label entirely, set `label == "suppressed"`.
#' @inheritParams predict_fit_and_ci
#' @param type Output from `process_model_type`.
...
zero_range <- function(x, tol = 0.01) {
  if (length(x) == 1)
    return(TRUE)
  y <-
    (max(x, na.rm = TRUE) - min(x, na.rm = TRUE)) / median(x, na.rm = TRUE)
  isTRUE(y < tol)
}
