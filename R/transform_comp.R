...
#'
#' @description
#' Takes a dataset with a set of labelled compositional columns and returns a
#' dataset with transformed variables added. The current behaviour also returns
#' compositional columns rescaled to be on the unitless scale, and with zeroes
#' imputed if `rounded_zeroes` is `TRUE`.
#'
#' @param data Dataset to have the transformation applied to.
#' @param comp_labels The labels of the compositional columns.
#' @param transformation_type The type of transformation desired. Should be
#' `"ilr"`, `"alr"` or `"clr"`.
#' @inheritParams process_zeroes
#' @param comparison_part Only needed for alr transformation. Should be an
#' element of `comp_labels`. Name of part that all other parts will be compared
#' to.
#' @param part_1 Used with ilr-pivot coordinate transformation to specify which
#' should be first part. Passed to `alter_order_comp_labels` unless `NULL`.
#'
#' @return
...
transform_comp <- function(
  ...
  # Checks
  transformation_type <- match.arg(transformation_type, c("alr", "clr", "ilr"))
  ...
  dTransformationReady$row_labels_master <- seq_len(nrow(dTransformationReady))
